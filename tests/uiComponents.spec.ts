...
test.describe('Form Layout Page', () => {
    ...
    test('Input Fields', async({page}) => {
        ...
        await usingTheGridEmailInput.fill('test@teste.com')
        await usingTheGridEmailInput.clear()
        await usingTheGridEmailInput.pressSequentially('test2@teste.com', {delay: 500})

        //generic assertion
        const inputValue = await usingTheGridEmailInput.inputValue()
        expect(inputValue).toEqual('test2@teste.com')

        //Locator assertion
        await expect(usingTheGridEmailInput).toHaveValue('test2@teste.com')
    })
...
test('CHECKBOX', async({page}) => {
        ...
        const allBoxes = page.getByRole('checkbox') //percorrer todas e marcar/desmarcar
...
test('Listas e dropdowns', async({page}) => {
